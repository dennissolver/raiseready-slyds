...
export default function SlydsThesisPage() {
  const [thesis, setThesis] = useState<SlydsThesis | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [isEditing, setIsEditing] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [editForm, setEditForm] = useState<Partial<SlydsThesis>>({});
  ...
  const updateArrayField = (field: keyof SlydsThesis, value: string) => {
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        {thesis.investment_philosophy && (
          <div className="mb-6 flex items-center gap-2 text-sm text-green-600">
            ...
            <Link href="/slyds/discovery" className="text-primary hover:underline ml-2">
              Refine with AI →
            </Link>
          </div>
        )}
            ...
            <CardContent>
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-lg">{thesis.firm || 'SlydS'}</p>
                </div>
              </div>
            </CardContent>
          </Card>
